...
cell_type_to_check = ['Immune (alveolar macrophage)', 'Epithelial cell (alveolar type I)']
cov_names = ['CoarseCellType']
cov_values = ['Immune']
cov_values_cf = ['Epithelial cell']

n_samples_from_source = min(n_samples_from_source_max, len(adata[(adata.obs['sex'] == 'male') &
                   (adata.obs['tissue'] == 'lingula of left lung') & 
                   (adata.obs['Broad cell type'] == cov_values[0])]))
...
deg_list = adata.uns['rank_genes_groups_split_3']['Epithelial cell (alveolar type I)']
r2_results = {}
