...
def biolord_pred(
    ...
    adata_ = adata[(adata.obs['sex'] == 'male') &
...
dis2p_model_path = (
    ...
    f'pretrainAE_10_maxEpochs_1000_split_{split_name}_reconW_20_cfWeight_1.5_beta_0.003_clf_0.8_adv_0.015_advp_5_n_cf_1_lr_0.01_wd_0.0005_new_cf_True_dropout_0.2_n_hidden_128_n_latent_32_n_layers_2batch_size_256_NoCT'
)
...
cov_names = ['tissue']
cov_values = ['anterior wall of left ventricle']
cov_values_cf = ['lingula of left lung']

n_samples_from_source = min(n_samples_from_source_max, len(adata[(adata.obs['sex'] == 'male') &
                   (adata.obs['tissue'] == cov_values[0]) & 
                   (adata.obs['Broad cell type'].isin(cell_type_to_check))]))
