...
adata = sc.read_h5ad(
    '/lustre/scratch126/cellgen/team205/aa34/Arian/Dis2P/eraslan_preprocessed1212_split_deg.h5ad')
adata = adata[adata.layers['counts'].sum(1) != 0].copy()

adata = adata[adata.obs[split_key] == 'train'].copy()
# Dummy observation for batch (We treat Sample ID/Batch as covariate)
...
torch.save(model.state_dict(), '/lustre/scratch126/cellgen/team205/aa34/Arian/Dis2P/models/scDisInfact/' +
           f"eraslan_scdisinfact_NoCT_defaultSettings_f{split_key}.pth")
