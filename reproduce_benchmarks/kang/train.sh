...
for split in "${split_key[@]}"; do
    for d in $model_scripts; do
        echo "python ${d} \"${split}\"" | \
            bsub -J ${d}_\"${split}\" -G teichlab -o logfile-${d}_\"${split}\".out -e logfile-${d}_\"${split}\".err -q gpu-lotfollahi -n1 -M16000 -R"select[mem>16000] rusage[mem=16000]" -gpu "mode=shared:num=1:gmem=12000"
        done