...
adata = sc.read_h5ad('/lustre/scratch126/cellgen/team205/aa34/Arian/Dis2P/hepatocyte_5000_split.h5ad')
adata.X = adata.layers['counts'].copy()
adata = adata[adata.X.sum(1) != 0].copy()

cats = ['time_int', 'status_control', 'zone']
split_key = 'split_1'
...
module_name = f'hepatocyte_ablation_celldisect_cfW'
...
model_name =  f'celldisect_{split_key}_cfW_{cf_weight}'
wandb_logger = WandbLogger(project=f"CellDISECT_{split_key}_AblationCF_hepatocyte", name=model_name)
