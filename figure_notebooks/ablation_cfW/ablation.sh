...
for d in $cfweights; do
    echo "python train_script.py ${d}" | \
        bsub -J ablation_cfW${d} -G teichlab -o logfile-${d}.out -e logfile-${d}.err -q gpu-normal -n2 -M16000 -R"select[mem>16000] rusage[mem=16000]" -gpu "mode=shared:num=1:gmem=10000"
    done