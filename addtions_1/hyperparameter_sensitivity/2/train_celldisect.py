...
# Handle command-line arguments for hyperparameter search
if len(sys.argv) > 1 and sys.argv[1] != 'default':
    train_dict['clf_weight'] = float(sys.argv[1])
if len(sys.argv) > 2 and sys.argv[2] != 'default':
    train_dict['adv_clf_weight'] = float(sys.argv[2])
if len(sys.argv) > 3 and sys.argv[3] != 'default':
    train_dict['cf_weight'] = float(sys.argv[3])
