...
def _process_spearman_columns(df, spearman_stat, metric_name):
    ...
    df : pd.DataFrame
        ...
        OR columns like cf_0.1_min, clf_0.01_mean, adv_0.1_max, etc.
    spearman_stat : str
    ...
    --------
    pd.DataFrame
        DataFrame with columns renamed to method names (CellDISECT, cf_0.1, etc.)
    """
    # Define the hyperparameter methods (new format)
    hyperparameter_methods = [
        'cf_0.1', 'cf_0.5', 'cf_1.0',
        'clf_0.01', 'clf_0.1', 'clf_0.5', 'clf_1.0', 
        'adv_0.01', 'adv_0.1', 'adv_0.5'
    ]
    
    # Define the old CellDISECT methods (for backward compatibility)
    old_methods = ['CellDISECT_1layer', 'CellDISECT_2layer', 'CellDISECT_4layer', 
                   'CellDISECT_6layer', 'CellDISECT_lr0.006', 'CellDISECT_lr0.01', 
                   'CellDISECT_lr0.03', 'CellDISECT_lr0.06', 'CellDISECT_lr0.1']
    
    # Combine all methods
    all_methods = hyperparameter_methods + old_methods
    
    if metric_name == 'Spearman':  # Regular spearman includes Control
        all_methods.append('Control')
    ...
    if columns_found:
        ...
        other_cols = [col for col in df.columns if not any(col.startswith(f"{m}_") for m in all_methods)]
...
def create_eraslan_config():
    ...
    return {
        ...
        'id_vars': ['Split'],
        'value_columns': ['Control', 'cf_0.1', 'cf_0.5', 'cf_1.0', 
                   'clf_0.01', 'clf_0.1', 'clf_0.5', 'clf_1.0', 
                   'adv_0.01', 'adv_0.1', 'adv_0.5',
                   'CellDISECT_1layer', 'CellDISECT_2layer', 'CellDISECT_4layer', 
                   'CellDISECT_6layer', 'CellDISECT_lr0.006', 'CellDISECT_lr0.01', 
                   'CellDISECT_lr0.03', 'CellDISECT_lr0.06', 'CellDISECT_lr0.1'],  # Include both new and old formats
        'categorical_order': {
            'Split': [
...
# 3. Eraslan dataset
csv_path_eraslan_old = 'cf_metrics/old/round2'
csv_path_eraslan_new = 'cf_metrics/new/round2'
...
custom_palette = {
        ...
        # New hyperparameter colors
        'cf_0.1': default_palette[0],     # Blue
        'cf_0.5': default_palette[1],     # Orange
        'cf_1.0': default_palette[2],     # Green
        'clf_0.01': default_palette[0],   # Blue
        'clf_0.1': default_palette[1],    # Orange
        'clf_0.5': default_palette[2],    # Green
        'clf_1.0': default_palette[3],    # Red
        'adv_0.01': default_palette[0],   # Blue
        'adv_0.1': default_palette[1],    # Orange
        'adv_0.5': default_palette[2],    # Green
    }
...
def plot_aggregation_cf_models(
    df,
    genes,
    save_name,
    spearman_stat_display=None,
    ...
    """
    Plot aggregated metrics for cf_ hyperparameter models only
    """
    # Filter for cf_ models
    cf_models = ['Control', 'cf_0.1', 'cf_0.5', 'cf_1.0']
    cf_pal = [custom_palette[model] for model in cf_models]
    ...
    data = df[df['Genes'] == genes].copy()
    data = data[data['method'].isin(cf_models)].copy()
    ...
    # Add title based on genes
    gene_title = "Top 20 Genes" if genes == '20' else "All Genes"
    fig.suptitle(f'CF Weight - {gene_title}', fontsize=16, y=0.95)
    
    # Adjust layout
    plt.tight_layout()
    
    # Save figures
    plt.savefig(f'cf_metrics/supp_plots_rev1/{save_name}_cf.svg', bbox_inches='tight', dpi=300) 
    plt.savefig(f'cf_metrics/supp_plots_rev1/{save_name}_cf.png', bbox_inches='tight', dpi=300) 
    plt.show()
    plt.clf()

def plot_aggregation_clf_models(
    df,
    genes,
    save_name,
    spearman_stat_display=None,
    ...
    """
    Plot aggregated metrics for clf_ hyperparameter models only
    """
    # Filter for clf_ models
    clf_models = ['Control', 'clf_0.01', 'clf_0.1', 'clf_0.5', 'clf_1.0']
    clf_pal = [custom_palette[model] for model in clf_models]
    ...
    data = df[df['Genes'] == genes].copy()
    data = data[data['method'].isin(clf_models)].copy()
    ...
    # Add title based on genes
    gene_title = "Top 20 Genes" if genes == '20' else "All Genes"
    fig.suptitle(f'CLF Weight - {gene_title}', fontsize=16, y=0.95)
    
    # Adjust layout
    plt.tight_layout()
    
    # Save figures
    plt.savefig(f'cf_metrics/supp_plots_rev1/{save_name}_clf.svg', bbox_inches='tight', dpi=300) 
    plt.savefig(f'cf_metrics/supp_plots_rev1/{save_name}_clf.png', bbox_inches='tight', dpi=300) 
    plt.show()
    plt.clf()

def plot_aggregation_adv_models(
    df,
    genes,
    save_name,
    spearman_stat_display=None,
    ...
    """
    Plot aggregated metrics for adv_ hyperparameter models only
    """
    # Filter for adv_ models
    adv_models = ['Control', 'adv_0.01', 'adv_0.1', 'adv_0.5']
    adv_pal = [custom_palette[model] for model in adv_models]
    ...
    data = df[df['Genes'] == genes].copy()
    data = data[data['method'].isin(adv_models)].copy()
    ...
    # Add title based on genes
    gene_title = "Top 20 Genes" if genes == '20' else "All Genes"
    fig.suptitle(f'ADV Weight - {gene_title}', fontsize=16, y=0.95)
    
    # Adjust layout
    plt.tight_layout()
    
    # Save figures
    plt.savefig(f'cf_metrics/supp_plots_rev1/{save_name}_adv.svg', bbox_inches='tight', dpi=300) 
    plt.savefig(f'cf_metrics/supp_plots_rev1/{save_name}_adv.png', bbox_inches='tight', dpi=300) 
    plt.show()
    plt.clf()

# Generate all hyperparameter plots
print("Generating hyperparameter plots...")

# CF hyperparameter plots
# print("Plotting CF hyperparameter - 20 genes...")
# plot_aggregation_cf_models(eraslan, '20', 'eraslan_20')

# print("Plotting CF hyperparameter - all genes...")
# plot_aggregation_cf_models(eraslan, 'all', 'eraslan_all')

# # CLF hyperparameter plots  
# print("Plotting CLF hyperparameter - 20 genes...")
# plot_aggregation_clf_models(eraslan, '20', 'eraslan_20')

# print("Plotting CLF hyperparameter - all genes...")
# plot_aggregation_clf_models(eraslan, 'all', 'eraslan_all')

# ADV hyperparameter plots
print("Plotting ADV hyperparameter - 20 genes...")
plot_aggregation_adv_models(eraslan, '20', 'eraslan_20')

print("Plotting ADV hyperparameter - all genes...")
plot_aggregation_adv_models(eraslan, 'all', 'eraslan_all')

print("All hyperparameter plots generated!")